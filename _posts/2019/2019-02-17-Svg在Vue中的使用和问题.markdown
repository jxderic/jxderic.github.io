...
想到这个主题是因为工作中遇到了icon使用的不便和前段时间改缺陷遇到的兼容性问题。
...
![雪碧图](http://thyrsi.com/t6/671/1550395610x2890174375.png)
...
在园区检索组件的集成测试中发现图标点击切换后会导致页面卡住，后面经过排除测出只有Windows7的IE上面才会出现这种情况，再一层层排查发现是svg里面的use标签在Windows7的IE中不支持点击。
